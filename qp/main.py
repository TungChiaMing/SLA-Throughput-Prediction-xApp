...
def qp_predict_handler(self, summary, sbuf):
    ...
    """
    #logger.debug("predict handler received payload {} from sla-spa".format(summary[rmr.RMR_MS_PAYLOAD]))

    logger.debug("predict handler received messages from sla-spa")
    ...
    #logger.debug("Sending message to sla-spa : {}".format(pred_msg))  # For debug purpose

    logger.debug("Sending response to sla-spa")  # For debug purpose
    if success:
    ...
    for cid in cell_list:
        ...
        if len(db.data) != 0:
            ...
            if df_f is not None:
                ...
                for pdcp in df_pred['pdcpBytesDl_predict']:
                    logger.debug("Predicted pdcpBytesDl is {}".format(pdcp))
                ...
                if(Do_Prediction_One_time == False):
                    db.write_prediction(df_pred_upload, 'QP_Prediction',pred=True)
...
def start(thread=False):
    ...
    if not thread:
        ...
        logger.debug("Populate Dummy Cell Data to InfluxDB")
        db = DATABASE('UEData')
    else:
        db = DUMMY()
